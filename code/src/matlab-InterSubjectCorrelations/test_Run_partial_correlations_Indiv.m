...
for i = 1:20
    ...
    curr_blind_data = data_blind(:, [1:i-1, i+1:end]);

    % Calculate the correlation between the current column and the average
    blind_WG_WholeBrain(i) = mean(corr(data_blind(:, i), curr_blind_data, 'Type',corrtype));
...
for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    curr_sighted_data = data_sighted(:, [1:i-1, i+1:end]);
    % Calculate the correlation between the current column and the average
    sighted_WG_WholeBrain(i) = mean(corr(data_sighted(:, i), curr_sighted_data, 'Type',corrtype));
...
for i = 1:20
    ...
    % Calculate the correlation between the current column and the average
    blind_BG_WholeBrain(i) = mean(corr(data_blind(:, i), data_sighted, 'Type',corrtype ));
end
...
for i = 1:20
    ...
    % Calculate the correlation between the current column and the average
    sighted_BG_WholeBrain(i) = mean(corr(data_sighted(:, i), data_blind, 'Type',corrtype));
end
...
for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    curr_blind_data = vis_cort_blind(:, [1:i-1, i+1:end]);

    % Calculate the correlation between the current column and the average
    blind_WG_OccipitalCortex(i) = mean(corr(vis_cort_blind(:, i), curr_blind_data,'Type',corrtype));
...
for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    curr_sighted_data = vis_cort_sighted(:, [1:i-1, i+1:end]);
    % Calculate the correlation between the current column and the average
    sighted_WG_OccipitalCortex(i) = mean(corr(vis_cort_sighted(:, i), curr_sighted_data, 'Type',corrtype));
...
for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    %curr_sighted_data = mean(vis_cort_sighted, 2);

    % Calculate the correlation between the current column and the average
    blind_BG_OccipitalCortex(i) = mean(corr(vis_cort_blind(:, i), vis_cort_sighted, 'Type',corrtype));
end

for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    %curr_blind_data = mean(vis_cort_blind, 2);

    % Calculate the correlation between the current column and the average
    sighted_BG_OccipitalCortex(i) = mean(corr(vis_cort_sighted(:, i), vis_cort_blind, 'Type',corrtype));
...
for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    curr_blind_data = v1_blind(:, [1:i-1, i+1:end]);

    % Calculate the correlation between the current column and the average
    blind_WG_V1(i) = mean(corr(v1_blind(:, i), curr_blind_data,'Type',corrtype));
end

% Loop through each column for sighted data
for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    curr_sighted_data = v1_sighted(:, [1:i-1, i+1:end]);
    % Calculate the correlation between the current column and the average
    sighted_WG_V1(i) = mean(corr(v1_sighted(:, i), curr_sighted_data, 'Type', corrtype));
end
...
for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    %curr_sighted_data = mean(v1_sighted, 2);

    % Calculate the correlation between the current column and the average
    blind_BG_V1(i) = mean(corr(v1_blind(:, i), v1_sighted));
end

for i = 1:20
    % Exclude the current column and calculate the average of the remaining columns
    %curr_blind_data = mean(v1_blind, 2);
